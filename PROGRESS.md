...
- T-010: **経路が端から端までつながった**（2026-09-19）。封筒 (b13994e)、storectl の join 動詞 (1d47100)、
  GUI 側の一回限りのパイプ (ba77081)、停止→更新→起動の順序 (dff8e35)、storectl のインストール配置
  (57e5d8e)、受け渡しの期限 (f155d24)、滞留による反映判定 (6ff46ac)、ウィザードの結線 (432f62a)。
  **残るのは人が UAC を押す昇格ありの実測だけ。**
...
- 2026-09-19: T-024 完了（M3.5 の速度判定を再利用の機構に合わせて置き直した）。
  T-023 は原因を特定（`attestation-failed` は結果で、真因は `/GL` の一時ファイルが作れない D8037。
  ゲート自身も `both-failed-common-boundary-consistent` として VFS の外側と判定している）。
  T-011 はアクション専用のステーションとデスクトップを実装し、機構をローカルで実証した。
- T-011 で分かった環境の制約: **対話セッションではウィンドウステーションを作成できない**
  （NULL 記述子でも ERROR_ACCESS_DENIED）。ローカルではフォールバックでブローカーのステーションを
  継承するので、Session 0 の経路は CI が初出になる。
