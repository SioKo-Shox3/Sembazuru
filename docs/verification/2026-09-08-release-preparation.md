...
- [T-001](../../blocked/T-001.md): ローカルでは cmd 経由の診断レコード往復テストも environment text で失敗する。依存更新自体は保持し、検証全体は未完了。
- [T-003](../../blocked/T-003.md): Session 0 診断 workflow が default branch に登録されておらず、実測は未実施。
- installed worker の 0xC0000142、C++/M2、Rust 1.98.1 の lint、Release の診断テストを解決する必要がある。
