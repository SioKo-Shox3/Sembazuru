...
#[cfg(windows)]
const LEGACY_TOKEN_DIAGNOSTIC: &str = "canonical worker config contains legacy cluster_token; stop SembazuruDaemon and SembazuruWorker, then run `sembazuru-storectl migrate-token`";
#[cfg(windows)]
...
impl WorkerConfigLocation {
    ...
    #[cfg(windows)]
    pub fn load_effective_checked(&self) -> Result<WorkerConfig, String> {
        self.load_effective_checked_with_reader(sembazuru_config_store::read_machine_cluster_token)
    }

    #[cfg(not(windows))]
    pub fn load_effective_checked(&self) -> Result<WorkerConfig, String> {
        WorkerConfig::load_effective_checked(&self.path())
    }
    ...
    fn load_effective_checked_with_reader<S: AsRef<[u8]>, E>(
        ...
        match self {
            Self::Canonical => {
                WorkerConfig::load_canonical_with_reader(&WorkerConfig::default_path(), read_secret)
            }
            Self::Override(path) => WorkerConfig::load_effective_checked(path),
        }
    }
            ...
            Ok(bytes) => {
                ...
                let table: toml::Table = toml::from_str(s).map_err(|e| {
                    if reject_legacy_token {
                        "canonical worker config is invalid TOML; refusing to start".into()
                    } else {
                        format!(
                ...
                table.try_into().map_err(|e| {
                    format!("worker config {} is invalid TOML ({e}); refusing to start on defaults (no agent/token/VFS). Fix or remove it.", path.display())
                })
            }
        }
    }
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn worker_canonical_startup_rejects_legacy_toml_before_machine_secret_read() {
        for legacy in [
            ...
            let err = WorkerConfig::load_canonical_with_reader(&path, || {
    ...
    #[cfg(windows)]
    #[test]
    fn worker_canonical_startup_loads_strict_utf8_machine_secret_and_env_wins_last() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let path = write_test_config("listen_addr = \"127.0.0.1:6001\"\n");
        ...
        let loaded = WorkerConfig::load_canonical_with_reader(&path, read).unwrap();
    ...
    #[cfg(windows)]
    #[test]
    fn worker_canonical_startup_rejects_non_utf8_or_reader_error_without_echo() {
        let path = write_test_config("");
        let invalid = WorkerConfig::load_canonical_with_reader(&path, || {
        ...
        let failed = WorkerConfig::load_canonical_with_reader(&path, || {
        ...
        let malformed = WorkerConfig::load_canonical_with_reader(
            &path,
        ...
        assert_eq!(
            malformed,
            "canonical worker config is invalid TOML; refusing to start"
        );
    }

    #[cfg(windows)]
    #[test]
    fn worker_override_startup_preserves_toml_token_and_skips_machine_secret() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
