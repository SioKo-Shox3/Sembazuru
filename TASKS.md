...
- review: 独立評価 NEEDS_WORK。blocking 2 件（Join が描画スレッドを止める、helper を待つ上限が
  helper 自身の最悪時間より短い）と非阻害 1 件（待機失敗のエラーが取り消し処理に上書きされる）を
  0791290 で解消した。0791290 自体は未評価。証拠は .harness/T-019-review.txt。
- residual: 評価が挙げた未検査の経路は残っている。(1) 生存したまま接続しない helper、
  (2) 接続後に読まない相手へバッファを超えて送る場合、(3) サーバーを閉じたあとの残データの読み取り、
  (4) `confirm_running` の呼び出しそのもの（時計と状態照会を差し替えないと検査できない）。
  (1)〜(3) は相手役の実プロセスが要るので、昇格ありの実測と同じ機会にまとめて行う。
- 残り: 昇格を伴う実機の受け渡しは未実測。
...
- measured: ローカル (VS BuildTools 18 の vcvars64 下) では 8/9、median +30.9us、
  差は +22〜+50us。CI では 5/9、median +6.6us、ばらつきは ±17us 程度。**効果は本物だが、CI では
  雑音と同程度まで縮む。** 8/9 の符号検定は、真の勝率が 0.65 程度でも通る確率が低く、
  コードではなくマシンを測っている状態だった。同じコミットがローカルで通り CI で落ちたのがその証拠。
- decided: このゲートで本当に守るべきものは「再利用が実際に起きていること」で、それは
  `Assert-OpenLatencyScenario` の接続数（persistent は 1000 読み取りで最大2接続、forced は 900 以上）が
  統計なしで決定的に検査している。時間の比較に残る役割は「再利用は起きているが、再接続より明確に
  遅くなった」場合の検出だけで、これは片側の問いになる。
- resolution: 対の数を 10 から 22（使用 21）に増やして中央値の標準誤差を 4us 程度に落とし、判定を
  (1) 勝ちが 7/21 未満なら失敗、(2) 中央値の差が persistent 中央値の 5% を超えて負なら失敗、へ置き換えた。
  どちらも「本当は差が無い」実行が約 5% で落ちる位置に取ってある（公平なコインで P(wins<=6)≈5%）。
- measured: 再設計後のローカル実行は 21/21、median +31.3us、所要 21 秒（従来の約2倍）。
